...
var PathDemoGetExcelSheet = "excel/sheet"
...
type DemoBMServer interface {
	...
	GetExcelSheet(ctx context.Context, req *GetExcelSheetReq) (resp *GetExcelSheetResp, err error)
}
...
func demoGetExcelSheet(c *bm.Context) {
	p := new(GetExcelSheetReq)
	if err := c.BindWith(p, binding.Default(c.Request.Method, c.Request.Header.Get("Content-Type"))); err != nil {
		return
	}
	resp, err := DemoSvc.GetExcelSheet(c, p)
	c.JSON(resp, err)
}
...
func RegisterDemoBMServer(e *bm.Engine, server DemoBMServer) {
	...
	e.POST("excel/sheet", demoGetExcelSheet)
}
